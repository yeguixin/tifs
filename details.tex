...
\end{figure}
        ...
        \label{secction:shake}
         ...
         However, videos recorded by a hand-held device \FIXED{are} not always perfectly steady due to
...
\FIXED{Where} $T$ is a Transformation Matrix, $S^{'}$ is the coordinate of a point of the tracked trajectory, and $S$ is the resulted coordinate after the transformation.
For each video frame, our algorithm individually calculates the filming angle and \FIXED{performs} the transformation, because the filming angle may change across video frames.
...
\begin{figure}[!t]
    ...
    \subsubsection{Map the Tracked Trajectory to Candidate Patterns\label{section:identity}}
       ...
       To determine the thresholds, we have evaluated a range of possible values in our initial design experiments to \FIXED{choose} the best performing values.
