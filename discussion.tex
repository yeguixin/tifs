...
\begin{figure}[!t]
        ...
        \caption{\FIXED{Examples of our new pattern lock mechanism. A true pattern of solid lines is shown
        ...
        Our second change requires the user to draw some random pattern structures (e.g. the dash lines in b and c)  before or after drawing the true pattern.
        The first change makes it difficult for the tracking algorithm to identify which dots are skipped (e.g. the fingertip movement trajectory in d, e and f), and the second change
        ...
        effectiveness of the video-based attack described in this work.}
        }
...
before or after drawing the correct pattern, the user is asked to draw a random pattern to confuse the attacker (\FIXED{Figure~\ref{fig:protection} b, c and respectively their fingertip movement trajectory e, f}).
For the second change, we relax the rules for creating a pattern to allow a touching dot to be visited multiple times (\FIXED{Figure~\ref{fig:protection} b}). Further, for
