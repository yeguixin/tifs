...
\begin{figure}[!t]
        ...
        \subfigure{
             \begin{minipage}[t]{0.1\textwidth}
                \includegraphics[width=\textwidth]{fig/protection4.pdf}\\
                \centering \footnotesize (c)
             \end{minipage}
        }
        \caption{\FIXED{Examples of the variant pattern lock mechanism for defending against our video-based attack. Here The pattern in (a) is the true pattern, which illustrates that a previously unvisited dot can be bypassed if it is part of a horizontal, vertical or diagonal line segment of the pattern. The patterns in (b), (c) and (d) are the fingertip movement trajectory of the true pattern. The subtle difference between (a) and (b) is because phone camera cannot identify which are touched points. Before or after drawing the true pattern, the user can draw a random pattern shown in (b) and (c), which are marked with dash lines.}}
