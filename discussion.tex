...
\begin{figure}[!t]
        ...
        \FIXED{The solid lines in (b) and (c) mark the true patterns.
        %\FIXME{What are the solid lines at (b) and (c)?}
        In order to increase the number of candidate patterns, a previously unvisited dot, such as the top-middle smaller dot in (a) or (c), can be bypassed if it is part of a horizontal, vertical or diagonal line segment of the pattern.}
        %\FIXME{Have no idea of what the last sentence is talking about.}
        }
...
Our experiment \FIXED{shows} that only two out of the 30 patterns can be cracked by our attacking method.
