...
\FIXED{Aviv \emph{et al.} developed an state-of-the-art guessing algorithm to brute force pattern lock~\cite{Aviv2015Is}. Theirs results show that patterns which generated on both $3\times3$ and $4\times4$ grid size can still be guessed out within thousands of guesses. To ensure the quality of collected patterns, Aviv \emph{et al.} analyzed the impact of different collection methods for pattern lock~\cite{Aviv2016Anlyzing}. They found that patterns are subtle differences when use pan-and-paper surveys and online self-reporting surveys to collect patterns. They suggested that it needs to consider the demographic factors such as gender during collecting patterns.}
